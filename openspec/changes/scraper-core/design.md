...
| D14 | **A born-digital document is extracted with its identifiers and a distinct outcome, never omitted — and its PDF is retrievable through a second, id-harvesting step** | (a) drop the row entirely (status quo); (b) store the HTML viewer page as if it were the document; (c) derive the PDF request from the ids the detail page already carries | The documents grid mixes two delivery shapes: legacy rows carrying `idBin=` (302 to a real PDF) and rows rendered through `documentoSemLoginHTML.seam?ca=…&idProcessoDoc=…`, a `text/html` viewer. **Corrected 2026-09-05:** this design, `parsing/detail-page.ts`'s comment, the fixture header and `docs/RESEARCH.md` all previously asserted the viewer has "no PDF at all". It does. The viewer page renders a `Gerar PDF` JSF command link whose `jsfcljs` call submits its form with `ca` plus `idProcDocBin`, returning a real PDF. (a) makes the shortfall D13 measures unattributable — a missing row looks identical whether it was skipped by design or never fetched. (b) stores a viewer page as if it were the document, the "200 that lies" failure this design refuses everywhere else. (c) is impossible and is the trap worth recording: the PDF is keyed on **`idProcDocBin`, which is not `idProcessoDoc`** — one observed document carries `idProcessoDoc=11688717` on its viewer link and `idProcDocBin=11492580` on its PDF button, under an identical `ca`. Neither id derives from the other, and `idProcDocBin` appears nowhere on the detail page (verified against the captured fixture, which holds four born-digital rows and zero occurrences of it). Retrieval is therefore two-step — fetch the viewer, harvest the form id, `ca` and `idProcDocBin` from its own markup, then POST — the same harvest-don't-guess discipline S5f established for the search trigger. Each step is a request charged to the run budget. |
