...
| S6 | Optional, off-by-default second-pass frontier crawl over persisted seeds | PR 19 | `vitest run src/engine/frontier.test.ts src/adapters/trf5/seeds.test.ts src/engine/scraper.test.ts src/main.test.ts` | `pnpm scrape --frontier --dry-run` (manual smoke only; additive, off by default) plus a bounded live pair (`pnpm scrape` then `pnpm scrape --frontier`) — see apply-progress.md | Delete `src/engine/frontier.ts`, `src/adapters/trf5/seeds.ts`; revert `engine/scraper.ts`'s `frontierSeedHarvest` config field and its call site, `adapters/trf5/traversal.ts`'s `seedCpf` cursor field, `adapters/trf5/site.ts`'s `documentoParte` wiring, and `main.ts`'s frontier branch; phase-1 scrape behavior is unaffected (proven by the full pre-existing suite staying green), though `engine/scraper.ts` itself is touched — a disclosed deviation from this row's own pre-launch forecast, see the S6 section |
...
## S6: Frontier crawl — additive, off by default (1184 authored `src/` lines actual — over budget, disclosed; see apply-progress.md)

Demonstrates: an optional second pass that targets known gaps and self-limits, without touching phase-1 behavior.

**Deviation disclosed, not silently absorbed**: TRF5's declared seed kinds are
`['lawyerCpf', 'partyCpf']`, both backed by the existing `documentoParte` search field —
not design.md's illustrative `['oab', 'exactName']` labels. A real `numeroOAB` field
exists on the form (docs/RESEARCH.md §2.5) but was never inventoried by S3's "Complete
Search Form Field Set", and adding it now would touch every existing session/search
fixture outside this slice's scope. See `apply-progress.md`'s `## S6` section and
`adapters/trf5/seeds.ts`'s own doc comment for the full reasoning.

- [x] 6.1 RED `engine/frontier.test.ts`: plain `scrape` issues zero frontier searches and persists seeds to `AdapterStateStore`; `scrape --frontier` in a new process reads a prior run's store without that process still running.
- [x] 6.2 GREEN implement `engine/frontier.ts` runner; wire `--frontier` in `cli/args.ts`.
- [x] 6.3 RED `adapters/trf5/seeds.test.ts`: OAB seed kind ranks above name; seeds are exact-match identifiers harvested from detail pages. **Amended**: `lawyerCpf` ranks above `partyCpf` (disclosed deviation above) — both are exact-match identifiers harvested from detail pages, satisfying the requirement's substance.
- [x] 6.4 GREEN implement `adapters/trf5/seeds.ts` (`FrontierCapable`: `seedKindRanking`, `harvestSeeds`, `unitFromSeed`).
- [x] 6.5 RED (extend `frontier.test.ts`): truncated-cell seeds are scheduled before complete-cell seeds regardless of kind ranking.
- [x] 6.6 GREEN implement queue ordering in `engine/frontier.ts`.
- [x] 6.7 RED (extend `frontier.test.ts`): a rolling window of zero-new-item seed searches stops further searches (yield decay).
- [x] 6.8 GREEN implement yield-decay tracking.
- [x] 6.9 RED (extend `frontier.test.ts`): the Nth request stops the run even while yield has not decayed.
- [x] 6.10 GREEN wire `engine/budget.ts` (S5b) into the frontier loop as a hard ceiling.
- [x] 6.11 RED (extend `frontier.test.ts` + `search.test.ts`): a seed search without a date range is rejected before send; a saturated seed search bisects via the same `traversal.ts` split used in phase 1. The "rejected before send" half reuses `search.ts`'s existing, already-passing `validateSearchCriteria` test generically — `unitFromSeed` always populates `RunBounds`' mandatory `dateFrom`/`dateTo`, so no seed-specific request can ever omit them; no redundant test added.
- [x] 6.12 GREEN wire date-range validation and split reuse into the frontier seed-search path.
- [x] 6.13 GREEN state, in `cli/summary.ts` output and README §Frontier, that frontier-crawl coverage gains are unmeasured and self-reinforcing.
