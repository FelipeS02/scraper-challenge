...
import { FakeNonDateSite, type FakeRegionItem } from '../support/fake-non-date-site.js';
import { FakeNonDateTraversal } from '../support/fake-non-date-traversal.js';
...
describe('module-graph seam check (non-date fake)', () => {
  it('never imports adapters/trf5, axios, or cheerio from the non-date fixture files', () => {
    ...
    const files = ['fake-non-date-site.ts', 'fake-non-date-traversal.ts'].map((name) =>
      join(engineDir, 'tests', 'support', name),
    );
    ...
    expect(readdirSync(join(engineDir, 'tests', 'support'))).toEqual(
